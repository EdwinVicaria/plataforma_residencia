...
<div class=" flex flex-col sm:justify-center items-center  bg-gray-100">
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-3xl">
        <form action="{{ route('users.update', $user->id) }}" method="post">
            ...
            <label class="block mb-2 text-sm font-medium text-gray-900">Nombre del usuario</label>
            <div class="relative">
                ...
                <input type="text"name="name"  placeholder="Ingrese su nombre" value="{{ old('name',$user->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  ">
            </div>
            @if ($errors->has('name'))
                <span class="error text-danger" for="input-password">{{ $errors->first('name') }}</span>
            @endif
            ...
            <div class="relative">
                ...
                <input type="text" name="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  " value="{{ old('name',$user->email) }}" placeholder="name@ejemplo.com">
            </div>
            @if ($errors->has('email'))
            ...
            <label class="block mb-2 text-sm font-medium text-gray-900" for="">Selecciona el rol para el usuario</label>
            <table>
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div>
                                <label for="">
                                    <input type="checkbox" name="roles[]" value="{{$id}}" {{ $user->roles->contains($id) ? 'checked' : ''}}>

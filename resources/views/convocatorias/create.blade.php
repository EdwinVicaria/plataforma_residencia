...
@section('contenido')
    ...
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0">
        ...
        <div class="bg-red-400 rounded-lg ">
            ...
            <form method="POST" action="{{route('convocatorias.store')}}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mt-5">
                    ...
                    <textarea name="descripcionCorta" id="descripcionCorta" cols="30" rows="10" class="block mt-1 w-full h-20 border border-black rounded-lg p-2"></textarea>
                    @if ($errors->has('descripcionCorta'))
                        <span class="text-red-500 font-medium" for="input-password">{{ $errors->first('descripcionCorta') }}</span>
                    @endif
                </div>
                <div class="mt-5">
                    <label for="descripcionLarga" >Descripción larga para leer más</label>

                    <textarea name="descripcionLarga" id="descripcionLarga" cols="30" rows="10" class="block mt-1 w-full border border-black rounded-lg p-2"></textarea>
                    @if ($errors->has('descripcionLarga'))
                        <span class="text-red-500 font-medium" for="input-password">{{ $errors->first('descripcionLarga') }}</span>
                    @endif
                </div>
                ...
                <div class="block mt-1 w-full rounded-lg mt-3 p-2">
                    ...
                    @if ($errors->has('imagen'))
                        <span class="text-red-500 font-medium" for="input-password">{{ $errors->first('imagen') }}</span>
                    @endif
                </div>
                ...
                <div class="mt-5">
                    ...
                    <input type="datetime-local" name="fechalimite" id="fechalimite" class="block mt-1 w-full border border-black rounded-lg p-2">
                    @if ($errors->has('fechalimite'))
                        <span class="text-red-500 font-medium" for="input-password">{{ $errors->first('fechalimite') }}</span>
                    @endif
                </div>
                ...
                <div align="center">
                    <a href="{{ route('convocatorias.index') }}" class="mt-3 p-2 border bg-red-300 hover:bg-red-500 rounded-lg text-white">Cancelar</a>
                    <button class="mt-3 p-2 border bg-yellow-600 hover:bg-yellow-700 rounded-lg text-white">
                        Crear convocatoria
                    </button>
                </div>
